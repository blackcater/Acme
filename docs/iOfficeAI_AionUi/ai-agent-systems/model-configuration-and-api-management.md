...
if (base_url && isMiniMaxAPI(base_url)) {
  const minimaxModels = [
    'MiniMax-M2.1', 'MiniMax-M2.1-lightning', 'MiniMax-M2', 'M2-her'
  ];
...
if (platform?.includes('bedrock') && bedrockConfig?.region) {
  ...
  const claudeProfiles = response.inferenceProfileSummaries.filter(
    profile => profile.inferenceProfileId?.includes('anthropic.claude')
  );
...
const command = new ListInferenceProfilesCommand({});
const response = await bedrockClient.send(command);
const claudeProfiles = inferenceProfiles.filter(
  profile => profile.inferenceProfileId?.includes('anthropic.claude')
);
