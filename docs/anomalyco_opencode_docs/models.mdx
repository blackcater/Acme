...
Here the full ID is `provider_id/model_id`. For example, if you're using [OpenCode Zen](./zen), you would use `opencode/gpt-5.1-codex` for GPT 5.1 Codex.
...
Use the keybind `variant_cycle` to quickly switch between variants. [Learn more](./keybinds).
