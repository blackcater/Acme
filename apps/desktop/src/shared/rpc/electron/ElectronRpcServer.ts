...
class ElectronRpcRouterImpl implements RpcRouter {
    ...
    router(namespace: string): RpcRouter {
        return new ElectronRpcRouterImpl(this.server, `${this.prefix}/${namespace}`)
    }
}
...
export class ElectronRpcServer implements RpcServer {
    ...
    handle(
        ...
        const handlerFn = typeof optionsOrHandler === 'function'
            ? optionsOrHandler
            : maybeHandler!

        // 监听 invoke:xxx 通道，接收客户端调用
        this.ipcMain.on(`rpc:invoke:${eventPath}`, async (e: IpcMainEvent, payload: { invokeId: string; args: unknown[] }) => {
